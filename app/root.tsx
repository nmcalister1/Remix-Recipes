...
function AppNavLink({ to, children } : AppNavLinkProps){
  ...
  const isLoading = navigation.state === "loading" && navigation.location.pathname === path.pathname && navigation.formData === null
  return (
    <li className="w-16">
      <NavLink to={to}>
        {({ isActive }) => (
    ...
    <html lang="en">
      ...
      <body>
        <div className="p-4">
          ...
          {error instanceof Error ? <p className="my-4 font-bold">{error.message}</p> : null}
          <Link to="/" className="text-primary">Take me home</Link>
        </div>
      </body>
    </html>
  )
